...
function pairUp(student) {
    ...
    if (student.length % 2 === 0) {
        for (let i = 1; i <= student.length; i += 2) {
            group.push(`${student[i - 1]} dan ${student[i]}`);
        }
    } else {
        for (let i = 1; i < student.length; i += 2) {
            group.push(`${student[i - 1]} dan ${student[i]}`);
        }
        group.push(`${student[student.length - 1]} dan Instruktur`);
    }
    
    return group;
}
  
//[ 'Budi dan Badu', 'Indra dan Indri' ]
console.log(pairUp(["Budi", "Badu", "Indra", "Indri"]));

//[ 'Budi dan Badu', 'Indra dan Indri' 'James dan Instruktur']
